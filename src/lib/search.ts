...
export function normalizeNames(film: Film): Film {
  ...
  let properBrand = hasKey(brandMappings, normalizedBrand)
    ? brandMappings[normalizedBrand]
    : toTitleCase(film.brand, brandMappings);
  ...
  let properName = hasKey(filmMappings, normalizedName)
    ? filmMappings[normalizedName]
    : toTitleCase(film.name, filmMappings);
